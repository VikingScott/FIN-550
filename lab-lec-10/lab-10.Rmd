...
loocv.err1 <- cv.glm(data = housing.df, glmfit = glm1)
# cross-validated MSE 
loocv.err1$delta[1]
```
...
summary(glm2)
# compute the leave-one-out cross-validation predictor error
loocv.err2 <- cv.glm(data = housing.df, glmfit = glm2)
# cross-validated MSE 
loocv.err2$delta[1]
```
...
summary(glm3)
# compute the leave-one-out cross-validation predictor error
loocv.err3 <- cv.glm(data = housing.df, glmfit = glm3)
# cross-validated MSE 
loocv.err3$delta[1]
```
...
set.seed(1)
kfcv.err1 <- cv.glm(data = housing.df, glmfit = glm1,K = 5)
# cross-validated MSE 
kfcv.err1$delta[1]
```
...
# compute the 5-fold cross-validation predictor error
set.seed(1)
kfcv.err2 <- cv.glm(data = housing.df, glmfit = glm2,K = 5)
# cross-validated MSE 
kfcv.err2$delta[1]
```
...
set.seed(1)
# compute the 5-fold cross-validation predictor error
kfcv.err3 <- cv.glm(data = housing.df, glmfit = glm3,K = 5)
# cross-validated MSE 
kfcv.err3$delta[1]
```
