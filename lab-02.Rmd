...
Answer: Because of floating point precision issues, the result of `0.3 + 0.6` is not exactly `0.9`, likely `0.89999999` due to binary storage.
...
all.equal(mean(y_error), 0)

# (a) and (b)
(length(y_error) == length(y)) & all.equal(mean(y_error), 0)
```
...
trends_csv <- read_csv(
  file = url,
  skip = 3,
  col_names = c("Country", "Search Interest", "Sport"),
  trim_ws = TRUE
)

trends_csv <- trends_csv[1:1408, ]
...
```{r}
china_data <- trends_csv[trends_csv$Country == "China", ]
china_data

china_top <- china_data[which.max(china_data$`Search Interest`), ]
china_top$Sport
...
```{r}
top_sport <- function(name) {
  country_data <- trends_csv[trends_csv$Country == name, ]
  country_top <- country_data[which.max(country_data$`Search Interest`), ]
  return(country_top$Sport)
}
